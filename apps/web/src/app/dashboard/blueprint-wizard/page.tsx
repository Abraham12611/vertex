...
export default function BlueprintWizardPage() {
  ...
  return (
    <div className="flex justify-center items-center min-h-[80vh] bg-gradient-to-br from-[#F5F7FA] to-[#E9ECF0]">
      <form
        className="w-full max-w-2xl mx-auto p-10 rounded-3xl shadow-2xl border border-white/30 bg-white/40 backdrop-blur-md flex flex-col gap-8"
        ...
        <div className="flex items-center gap-4 mb-2">
          {steps.map((s, i) => (
            <div key={s.label} className="flex-1 flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-white transition-all ${i <= step ? 'bg-[#007AFF] shadow-lg' : 'bg-gray-300'}`}>{i + 1}</div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {current.fields.map(f => (
            <div key={f.name} className="flex flex-col gap-1">
              <label className="font-medium text-gray-700 mb-1">{f.label}{f.required && <span className="text-red-500">*</span>}</label>
              {f.type === "textarea" ? (
                <textarea
                  name={f.name}
                  value={form[f.name] || ""}
                  onChange={handleChange}
                  className="rounded-lg border border-gray-200 px-3 py-2 text-base font-sans bg-white/70 focus:ring-2 focus:ring-[#007AFF] shadow-inner"
                  rows={3}
                />
              ) : (
                <input
